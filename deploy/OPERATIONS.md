...
Keep reminder email disabled until migration `0011` is applied and the dedicated
LNemail and admin-NWC secrets are mounted. Reminder delivery does not require
...
`/admin*` and `/api/v1/admin/*` from every other source receive `404`.
...
Optional reminder delivery requires an LNemail account token and a separate,
budget-limited operator NWC URI. Create owner-only `lnemail-access-token` and
`lnemail-admin-nwc-uri` files, mount them only on the `backend` service, and set
`LNEMAIL_ACCESS_TOKEN_FILE` and `LNEMAIL_ADMIN_NWC_URI_FILE` to those mount paths.
The same `CREDENTIAL_ENCRYPTION_KEY_FILE` protects recipient addresses with a
distinct encryption purpose. Set `PAYMENT_NWC_ADAPTER=nwc`, an exact
`NWC_ALLOWED_RELAY_HOSTS` list, `LNEMAIL_MAX_SEND_PRICE_SATS=100`, and then
`REMINDER_EMAIL_ENABLED=true`. Do not mount the admin NWC URI in relay, Caddy,
HAProxy, PostgreSQL, or migration containers. Give that NWC connection a wallet-side
budget limited to reminder sends and routing fees, not subscription prices.
The Compose migration service forces reminders off so schema upgrades do not need
runtime LNemail or admin-wallet credentials. Add the three runtime secret mounts only
to `backend`, not to the shared secret anchor or `migrate` service.

The reconciler queues one notice seven days before expiry and one notice one day
before expiry. It encrypts both the recipient and provider BOLT11 because LNemail can
embed the recipient in the invoice description. A lost
LNemail create response is terminal because the provider has no idempotency key;
an ambiguous NWC payment is polled at LNemail and looked up by payment hash before
any retry. LNemail still receives the recipient and generated message in plaintext
and may retain that outgoing-send record under its own policy. Disabling reminders
cancels queued and created-but-unpaid work but cannot retract an in-flight provider
request or wallet payment. Review terminal and ambiguous delivery states in the admin UI.
