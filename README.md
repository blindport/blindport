...
payment path is receive-only Lightning through Coinos LNURL. CLINK Debits and Nostr
Wallet Connect are optional automatic wallet controls over the resulting BOLT11
invoice. CLINK is preferred when both are connected, with NWC fallback only after an
explicit CLINK rejection. Operators may separately enable stablecoin checkout.
Lightning Swap opens a snapshotted provider tab with the percent-encoded BOLT11
invoice; its callback cannot activate service. Blindport activates service only after
merchant LNURL verification confirms the invoice and preimage. Legacy persisted Cashu
payment rows remain readable, but Cashu runtime support has been removed.

An optional Lightning Address referral is captured only on first touch in the current
browser tab and remains bound to the subscription and renewals. Referrers do not need
a Blindport account. Eligible, merchant-verified LNURL payments create one credit from
the payment's snapshotted rate; payouts remain an operator-run external Lightning task.
...
| `docs/` | Architecture, protocol, and client documentation. |
...
Agent configuration is documented in [docs/agent.md](docs/agent.md).
Application code, development tooling, and customer examples are public.
Internal operations documentation and production deployment configuration are
maintained privately and are not included in this repository.
...
The development Compose stack is not a production deployment. New deployments
default to receive-only LNURL and require no LND credentials. An existing LND deployment must
run migration `0033` with its current configuration, stop creating invoices, drain all
pending LND invoices, and explicitly switch to LNURL before replacing its configuration.
...
Production receives payments through the fixed Coinos LNURL endpoint. It accepts only
signed, exact-amount mainnet BOLT11 invoices with the expected LNURL metadata hash,
and settles only after the provider verifies the invoice and matching preimage. LNURL
invoice creation has no idempotency key: Blindport records an attempt before the request
and never automatically issues another invoice after interruption, failure, or timeout.
The 600-second local checkout reservation can expire before the provider's invoice;
late verified payments require manual review and never grant service or referral credit.
...
merchant LNURL settlement verification. Because CLINK v1 has no payment lookup or mandatory idempotency,
an ambiguous post-send result is never retried or passed to NWC automatically.

Stablecoin checkout uses the same external invoice and merchant-verification path. Blindport
...
time rounded up to a whole day. Merchant LNURL settlement verification remains authoritative.
