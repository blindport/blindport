...
# Coincurve builds its bundled secp256k1 library on Python 3.14. Keep the
# compiler and Autotools in this build stage, outside the runtime image.
RUN apt-get update \
    && apt-get install -y --no-install-recommends build-essential autoconf automake libtool pkg-config libffi-dev \
