...
func main() {
	...
	challengeListen := flag.String("http-challenge", os.Getenv("BLINDPORT_RELAY_HTTP_CHALLENGE"), "HTTP-01-only listen address, for example :80 (empty disables it; safe behind an L7 frontend)")
	...
	maxChallenges := flag.Int("max-http-challenges", envIntDefault("BLINDPORT_RELAY_MAX_HTTP_CHALLENGES", 64), "maximum concurrent HTTP-01 requests")
	challengeRate := flag.Int("http-challenge-rate", envIntDefault("BLINDPORT_RELAY_HTTP_CHALLENGE_RATE", 600), "valid HTTP-01 requests allowed per minute per direct peer (counts requests, not certificates; requests behind one L7 frontend share its allowance)")
	challengeBurst := flag.Int("http-challenge-burst", envIntDefault("BLINDPORT_RELAY_HTTP_CHALLENGE_BURST", 100), "HTTP-01 per-peer burst allowance for retries, repeated requests, and multi-vantage validation")
