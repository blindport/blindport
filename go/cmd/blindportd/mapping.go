...
func validateMappings(mappings []mapping) error {
	seen := make(map[int64]string, len(mappings))
	for i, item := range mappings {
		...
		if item.SubscriptionID <= 0 {
			return fmt.Errorf("%s: subscription_id must be positive", source)
		}
		...
		if previous, ok := seen[item.SubscriptionID]; ok {
			return fmt.Errorf("duplicate subscription_id %d in %s and %s", item.SubscriptionID, previous, source)
		}
...
func claimFromProvisioning(row provisioning) (*protocol.Claim, error) {
	...
	if err := protocol.ValidateClaim(claim); err != nil {
		return nil, fmt.Errorf("subscription %d has invalid provisioned claim: %w", row.SubscriptionID, err)
	}
	return claim, nil
}
...
func buildMappingPlansWithMissing(mappings []mapping, cfg []provisioning, relayOverride string, allowMissing bool) ([]workerPlan, error) {
	...
	byID := make(map[int64]provisioning, len(cfg))
	for _, row := range cfg {
	...
	for _, item := range mappings {
		...
		if !ok {
			...
			return nil, fmt.Errorf("configured subscription %d does not exist or is not active", item.SubscriptionID)
		}
		...
		if item.HTTPChallengeUpstream != "" && claim.Kind != protocol.ClaimRelay {
			return nil, fmt.Errorf("subscription %d: http_challenge_upstream is only valid for Blindport Relay", item.SubscriptionID)
		}
		endpoints, err := provisioningEndpoints(row, relayOverride)
		if err != nil {
			return nil, fmt.Errorf("subscription %d: %w", item.SubscriptionID, err)
		}
