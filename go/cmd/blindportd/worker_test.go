...
func TestWorkerSupervisorAddUpdateRemoveAndShutdown(t *testing.T) {
	...
	first := workerPlan{SubscriptionID: 1, RelayAddr: "edge-a:5443", Upstream: "old:443", Claim: claim}
	second := workerPlan{SubscriptionID: 1, RelayAddr: "edge-b:5443", Upstream: "old:443", Claim: claim}
...
func TestWorkerSupervisorConcurrentReconcileIsRaceSafe(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		go func(i int) {
			defer callers.Done()
			plan := workerPlan{SubscriptionID: int64(i%3 + 1), RelayAddr: "edge:5443", Upstream: "app:80"}
...
func TestWorkerSupervisorRestartsWorkerThatReturns(t *testing.T) {
	...
	plan := workerPlan{SubscriptionID: 1, RelayAddr: "edge:5443", Upstream: "app:80"}
	if err := supervisor.Reconcile([]workerPlan{plan}); err != nil {
		t.Fatal(err)
	}
