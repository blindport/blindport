...
func TestOrderAPIClientSendsContractAndAcceptsFullResponse(t *testing.T) {
  ...
  "order_key":"web",
  "subscription":{"id":42,"status":"pending","product":"relay","monthly_price_sats":100},
  ...
  "state":"awaiting_domain"
	...
	if response.Subscription.ID != 42 || response.State != "awaiting_domain" {
...
func TestOrderAPIClientBoundsAndValidatesResponses(t *testing.T) {
	tests := map[string]struct {
		...
		"trailing":      {body: `{"order_key":"web","subscription":{"id":1,"status":"pending"},"state":"awaiting_payment"} {}`},
		"wrong key":     {body: `{"order_key":"other","subscription":{"id":1,"status":"pending"},"state":"awaiting_payment"}`},
		"invalid sub":   {body: `{"order_key":"web","subscription":{"id":0,"status":"pending"},"state":"awaiting_payment"}`},
		"unknown state": {body: `{"order_key":"web","subscription":{"id":1,"status":"pending"},"state":"surprising"}`},
	}
...
func TestDockerAgentPendingToActiveRepeatedSnapshotAndRemoval(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		putCalls++
		_, _ = io.WriteString(w, `{"order_key":"web","subscription":{"id":42,"status":"pending"},"payment":null,"state":"awaiting_payment"}`)
	}))
	defer server.Close()
	...
	cfg = []provisioning{{
		SubscriptionID: 42, Product: "relay", Domain: "web.example",
		RelayEndpoint: "edge.example:5443", Transport: "tcp",
	}}
	agent.reconcile(context.Background())
	calls := reconciler.snapshots()
	if len(calls) != 2 || len(calls[1]) != 1 || calls[1][0].SubscriptionID != 42 || calls[1][0].Upstream != "web:443" {
...
func TestDockerAgentRetainsSnapshotAndWorkersAcrossTransientFailures(t *testing.T) {
	fake := &fakeDockerClient{containers: []containertypes.Summary{{ID: "legacy", Labels: dockerLabels("web", "42", "web:443")}}}
	reconciler := &recordingPlanReconciler{}
	fetchErr := error(nil)
	agent := newTestDockerAgent(fake, nil, reconciler, func(context.Context) ([]provisioning, error) {
		...
		return []provisioning{{
			SubscriptionID: 42, Product: "relay", Domain: "web.example", RelayEndpoint: "edge.example:5443",
		}}, nil
	})
	agent.reconcile(context.Background())
...
func TestDockerAgentKeepsChangedOrderFallbackButStillAppliesRemoval(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		...
		_, _ = io.WriteString(w, `{"order_key":"web","subscription":{"id":42,"status":"active"},"state":"active"}`)
	}))
	defer server.Close()
	fake := &fakeDockerClient{containers: []containertypes.Summary{{ID: "web", Labels: testOrderLabels()}}}
	reconciler := &recordingPlanReconciler{}
...
func TestDockerAgentRetriesFailedAndAttentionOrdersWithBackoff(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		...
		_, _ = io.WriteString(w, `{"order_key":"web","subscription":{"id":42,"status":"pending"},"state":"`+response.state+`"}`)
	}))
	defer server.Close()
	fake := &fakeDockerClient{containers: []containertypes.Summary{{ID: "web", Labels: testOrderLabels()}}}
	reconciler := &recordingPlanReconciler{}
