...
EXPECTED_DATETIME_COLUMNS = {
    ...
    ("reminderdelivery", "invoice_created_at"),
    ("reminderdelivery", "paid_at"),
    ("reminderdelivery", "delivered_at"),
...
def test_sqlite_0009_backfills_billing_snapshots_and_supports_rolling_inserts(
    ...
    assert database_revisions(engine) == ("0008", "0012")
...
def test_sqlite_downgrade_and_upgrade_round_trip(tmp_path) -> None:
    ...
    assert database_revisions(engine) == (None, "0012")
...
def test_current_cli_can_check_head(tmp_path, monkeypatch, capsys) -> None:
    ...
    assert "current: 0012\nhead: 0012" in capsys.readouterr().out
    ...
    assert captured.err == (
        "error: database revision is unversioned, expected migration head 0012\n"
    )
