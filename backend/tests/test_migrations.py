...
EXPECTED_COLUMNS = {
    ...
    "payment": {
        ...
        "stablecoin_order_id",
        "stablecoin_order_token_ciphertext",
        "stablecoin_order_token_key_version",
        "stablecoin_order_status",
        "stablecoin_deposit_amount",
        "stablecoin_deposit_address",
        "stablecoin_deposit_network",
        "stablecoin_deposit_tag",
        "stablecoin_required_confirmations",
        "stablecoin_order_expires_at",
        "created_at",
        "paid_at",
        "expires_at",
    },
...
EXPECTED_DATETIME_COLUMNS = {
    ...
    ("subscription", "upgrade_source_period_end"),
    ...
    ("payment", "lnurl_attempted_at"),
    ("payment", "invoice_expires_at"),
    ("payment", "settlement_verified_at"),
    ("payment", "lnurl_last_checked_at"),
    ("referralcredit", "created_at"),
    ("referralpayout", "created_at"),
...
def test_fresh_sqlite_upgrade_current_and_schema(tmp_path) -> None:
    ...
    assert {fk["referred_table"] for fk in subscription_foreign_keys} == {"subscription", "user"}
    assert {
        constraint["name"] for constraint in inspector.get_unique_constraints("subscription")
    } >= {"uq_subscription_upgrade_from_subscription"}
    ...
    assert inspector.get_foreign_keys("relayedgedailybandwidth") == []
    assert {
...
def test_0014_upgrades_deployed_0013_subscription_identity(tmp_path) -> None:
    ...
    assert database_revisions(engine) == ("0013", "0033")
...
def test_0016_rejects_downgrade_with_stablecoin_payments(tmp_path) -> None:
    ...
    downgrade_database(engine, "0015")
    assert database_revisions(engine) == ("0015", "0033")
...
def test_0017_backfills_current_ip_lease_and_downgrades_cleanly(tmp_path) -> None:
    ...
    assert database_revisions(engine) == ("0016", "0033")
...
def test_0018_service_announcements_downgrade_cleanly_on_sqlite(tmp_path) -> None:
    ...
    assert database_revisions(engine) == ("0017", "0033")
    inspector = inspect(engine)
...
def test_0019_production_observations_downgrade_cleanly_on_sqlite(tmp_path) -> None:
    ...
    assert database_revisions(engine) == ("0018", "0033")
    inspector = inspect(engine)
...
def test_0020_relay_hostname_scope_defaults_existing_rows_and_downgrades_cleanly(tmp_path) -> None:
    ...
    assert database_revisions(engine) == ("0019", "0033")
...
def test_0021_daily_bandwidth_upgrade_and_downgrade_ordering(tmp_path) -> None:
    ...
    assert database_revisions(engine) == ("0020", "0033")
    assert not inspect(engine).has_table("subscriptiondailybandwidth")
    assert not inspect(engine).has_table("relaybandwidthcursor")
...
def test_0022_notification_outbox_downgrades_to_0021_on_sqlite(tmp_path) -> None:
    ...
    assert database_revisions(engine) == ("0021", "0033")
    inspector = inspect(engine)
...
def test_0023_passkey_persistence_downgrades_to_0022_on_sqlite(tmp_path) -> None:
    ...
    assert database_revisions(engine) == ("0022", "0033")
    inspector = inspect(engine)
...
def test_0025_relay_subscription_connections_upgrades_and_downgrades_cleanly(tmp_path) -> None:
    ...
    assert database_revisions(engine) == ("0024", "0033")
    assert not inspect(engine).has_table("relaysubscriptionconnection")
...
def test_0026_backfills_stablecoin_checkout_snapshots_and_downgrades_cleanly(tmp_path) -> None:
    ...
    assert database_revisions(engine) == ("0026", "0033")
    ...
    downgrade_database(engine, "0025")
    assert database_revisions(engine) == ("0025", "0033")
...
def test_0027_adds_guarded_encrypted_lightning_swap_order_state(tmp_path) -> None:
    engine = _sqlite_engine(tmp_path)
    upgrade_database(engine, "0026")
    payment = Table("payment", MetaData(), autoload_with=engine)
    subscription = Table("subscription", MetaData(), autoload_with=engine)
    ...
    with engine.begin() as connection:
        user_id = connection.execute(
            user.insert().values(
                hashed_token="lightning-swap-order-migration-user",
                is_admin=False,
                is_suspended=False,
                created_at=created_at,
            )
        ...
        subscription_id = connection.execute(
            subscription.insert().values(
                ...
                domain_is_managed=False,
                billing_term="monthly",
                monthly_price_sats=1500,
                yearly_price_sats=15000,
                auto_renew=False,
                created_at=created_at,
                updated_at=created_at,
            )
    ...
    assert columns["stablecoin_order_id"].type.length == 32
    assert columns["stablecoin_order_token_ciphertext"].type.length == 8192
    assert columns["stablecoin_order_token_key_version"].type.length == 32
    assert columns["stablecoin_order_status"].type.length == 32
    assert columns["stablecoin_surcharge_sats"].nullable is False
    with engine.connect() as connection:
        row = connection.execute(select(upgraded).where(upgraded.c.id == payment_id)).one()
    assert row.stablecoin_api_order_enabled is False
    assert row.stablecoin_order_id is None
    assert row.stablecoin_surcharge_sats == 0

    downgrade_database(engine, "0027")
    assert database_revisions(engine) == ("0027", "0033")
    ...
    with engine.begin() as connection:
        connection.execute(
            upgraded.update()
            .where(upgraded.c.id == payment_id)
            .values(stablecoin_api_order_enabled=True)
        )
...
def test_0028_adds_surcharge_defaults_and_guards_bonus_day_downgrades(tmp_path) -> None:
    ...
    with engine.begin() as connection:
        user_id = connection.execute(
            user.insert().values(
                hashed_token="stablecoin-surcharge-migration-user",
                is_admin=False,
                is_suspended=False,
                created_at=created_at,
            )
    ...
    assert database_revisions(engine) == ("0028", "0033")
    upgraded_payment = Table("payment", MetaData(), autoload_with=engine)
    assert upgraded_payment.c.stablecoin_surcharge_sats.nullable is False
    with engine.connect() as connection:
        legacy_payment = connection.execute(
    ...
    with engine.connect() as connection:
        assert (
            connection.execute(
                select(downgraded_payment.c.id).where(downgraded_payment.c.id == legacy_payment_id)
            ).scalar_one()
            == legacy_payment_id
        )
    ...
    upgraded_payment = Table("payment", MetaData(), autoload_with=engine)
    with engine.begin() as connection:
        bonus_subscription_id = connection.execute(
        ...
        connection.execute(
            upgraded_payment.insert().values(
                subscription_id=bonus_subscription_id,
                method="stablecoin_swap",
                status="pending",
                billing_term="monthly",
                period_days=31,
...
def test_0029_persists_upgrade_snapshots_and_guards_downgrades(tmp_path) -> None:
    engine = _sqlite_engine(tmp_path)
    upgrade_database(engine, "0028")
    metadata = MetaData()
    user = Table("user", metadata, autoload_with=engine)
    ...
    with engine.begin() as connection:
        user_id = connection.execute(
            user.insert().values(
                hashed_token="relay-wildcard-upgrade-migration-user",
                is_admin=False,
                is_suspended=False,
                created_at=created_at,
            )
        ).inserted_primary_key[0]
        source_subscription_id = connection.execute(
            subscription.insert().values(
                user_id=user_id,
                product="relay",
                delivery="FRAMED",
    ...
    assert database_revisions(engine) == ("0029", "0033")
    upgraded_subscription = Table("subscription", MetaData(), autoload_with=engine)
    upgraded_payment = Table("payment", MetaData(), autoload_with=engine)
    with engine.connect() as connection:
    ...
    assert (
        legacy_subscription.upgrade_from_subscription_id,
        legacy_subscription.upgrade_credit_sats,
        legacy_subscription.upgrade_source_period_end,
    ) == (None, 0, None)
    assert (legacy_payment.service_price_sats, legacy_payment.discount_sats) == (0, 0)
    assert any(
        fk["name"] == "fk_subscription_upgrade_from_subscription"
        and fk["constrained_columns"] == ["upgrade_from_subscription_id"]
        and fk["referred_table"] == "subscription"
        for fk in inspect(engine).get_foreign_keys("subscription")
    )
    assert {
        constraint["name"] for constraint in inspect(engine).get_unique_constraints("subscription")
    } >= {"uq_subscription_upgrade_from_subscription"}

    with engine.begin() as connection:
        target_subscription_id = connection.execute(
            upgraded_subscription.insert().values(
                user_id=user_id,
                product="relay",
                delivery="FRAMED",
                ...
                upgrade_credit_sats=125,
                upgrade_source_period_end=created_at,
                created_at=created_at,
                updated_at=created_at,
            )
        ).inserted_primary_key[0]
    with pytest.raises(IntegrityError), engine.begin() as connection:
        connection.execute(
            upgraded_subscription.insert().values(
                ...
                status="pending",
                transport="TCP",
                domain="duplicate-wildcard.example.test",
                domain_is_managed=False,
                billing_term="monthly",
                monthly_price_sats=1000,
                yearly_price_sats=10000,
        ...
        with pytest.raises(IntegrityError), connection.begin():
            connection.execute(
                upgraded_subscription.insert().values(
                    ...
                    status="pending",
                    transport="TCP",
                    domain="invalid-source.example.test",
                    domain_is_managed=False,
                    billing_term="monthly",
                    monthly_price_sats=1000,
                    yearly_price_sats=10000,
                    auto_renew=False,
                    upgrade_from_subscription_id=source_subscription_id + 1000,
                    created_at=created_at,
                    updated_at=created_at,
                )
            )
    ...
    with engine.begin() as connection:
        connection.execute(
            ...
            .where(upgraded_subscription.c.id == target_subscription_id)
    ...
    assert {
        ...
        "upgrade_credit_sats",
        "upgrade_source_period_end",
    }.isdisjoint(downgraded_subscription.c.keys())
    assert {"service_price_sats", "discount_sats"}.isdisjoint(downgraded_payment.c.keys())
    ...
    with engine.begin() as connection:
        connection.execute(
            upgraded_subscription.update()
            .where(upgraded_subscription.c.id == target_subscription_id)
            .values(upgrade_from_subscription_id=source_subscription_id)
        )
...
def test_0030_persists_prepared_deposit_snapshots_and_guards_downgrades(tmp_path) -> None:
    ...
    with engine.begin() as connection:
        user_id = connection.execute(
            user.insert().values(
                hashed_token="prepared-deposit-migration-user",
                is_admin=False,
                is_suspended=False,
                created_at=created_at,
            )
    ...
    assert {
        ...
        "stablecoin_deposit_amount": (128, True),
        "stablecoin_deposit_address": (512, True),
        "stablecoin_deposit_network": (32, True),
        "stablecoin_deposit_tag": (512, True),
    }
    assert columns["stablecoin_required_confirmations"].nullable is True
    with engine.connect() as connection:
        legacy = connection.execute(
            select(upgraded).where(upgraded.c.id == legacy_payment_id)
        ).one()
    assert (
        legacy.stablecoin_deposit_amount,
        legacy.stablecoin_deposit_address,
        legacy.stablecoin_deposit_network,
        legacy.stablecoin_deposit_tag,
        legacy.stablecoin_required_confirmations,
    ...
    with engine.begin() as connection:
        connection.execute(
            upgraded.update()
            .where(upgraded.c.id == legacy_payment_id)
            .values(stablecoin_api_order_enabled=True)
        )
    with pytest.raises(RuntimeError, match="Lightning Swap API payments exist"):
        downgrade_database(engine, "0029")
    ...
    with engine.begin() as connection:
        connection.execute(
            ...
            .where(upgraded.c.id == legacy_payment_id)
            .values(
                stablecoin_api_order_enabled=False,
...
def test_0031_backfills_daily_relay_edge_aggregates_and_downgrades_cleanly(tmp_path) -> None:
    engine = _sqlite_engine(tmp_path)
    upgrade_database(engine, "0030")
    metadata = MetaData()
    user = Table("user", metadata, autoload_with=engine)
    subscription = Table("subscription", metadata, autoload_with=engine)
    cursor = Table("relaybandwidthcursor", metadata, autoload_with=engine)
    created_at = datetime(2026, 1, 1, tzinfo=UTC)
    day = created_at.date()
    previous_day = (created_at - timedelta(days=1)).date()
    with engine.begin() as connection:
        user_id = connection.execute(
            user.insert().values(
                public_id=uuid4().hex,
                hashed_token="relay-edge-bandwidth-migration-user",
                is_admin=False,
                is_suspended=False,
                created_at=created_at,
            )
    ...
    assert inspect(engine).get_foreign_keys("relayedgedailybandwidth") == []
    with engine.connect() as connection:
        rows = connection.execute(
            select(edge_daily).order_by(edge_daily.c.edge_id, edge_daily.c.day)
        ).all()
    assert [(row.edge_id, row.day, row.ingress_bytes, row.egress_bytes) for row in rows] == [
        ("edge-a", previous_day, 15, 4),
        ("edge-a", day, 30, 10),
        ("edge-b", day, 30, 9),
    ]
    ...
    assert database_revisions(engine) == ("0030", "0033")
...
def test_0032_adds_clink_columns_evolves_enum_and_guards_downgrade(tmp_path) -> None:
    ...
    assert database_revisions(engine) == ("0031", "0033")
...
def test_sqlite_0009_backfills_billing_snapshots_and_supports_rolling_inserts(
    ...
    assert database_revisions(engine) == ("0008", "0033")
...
def test_sqlite_downgrade_and_upgrade_round_trip(tmp_path) -> None:
    ...
    assert database_revisions(engine) == (None, "0033")
...
def test_current_cli_can_check_head(tmp_path, monkeypatch, capsys) -> None:
    ...
    assert "current: 0033\nhead: 0033" in capsys.readouterr().out
    ...
    assert captured.err == (
        "error: database revision is unversioned, expected migration head 0033\n"
    )
