...
def test_reconciler_activates_paid_lightning_without_payment_get(app_client) -> None:
    ...
    from blindport.core.models import Payment, PaymentStatus, Subscription, SubscriptionStatus
    ...
    with Session(engine) as session:
        ...
        assert session.get(Subscription, subscription["id"]).status == SubscriptionStatus.ACTIVE  # type: ignore[union-attr]
...
def test_reconciler_expiry_releases_only_the_expired_payments_reservation(app_client) -> None:
    ...
    with Session(engine) as session:
        ...
        subscription = session.get(Subscription, first_subscription["id"])
    ...
    with Session(engine) as session:
        first_stored = session.get(Subscription, first_subscription["id"])
        second_stored = session.get(Subscription, second_subscription["id"])
...
def test_operator_reminders_run_without_customer_lightning_or_nwc_enabled(
    ...
    monkeypatch.setattr(
        ...
        lambda batch_size: ReminderReconciliationSummary(queued=2, delivered=1),
    )
    ...
    assert summary.reminders_delivered == 1
...
def test_repeated_reconciliation_does_not_double_renew(app_client) -> None:
    ...
    with Session(engine) as session:
        period_before = session.get(Subscription, subscription["id"]).current_period_end  # type: ignore[union-attr]

    reconcile_pending_payments_once()
    reconcile_pending_payments_once()

    with Session(engine) as session:
        period_after = session.get(Subscription, subscription["id"]).current_period_end  # type: ignore[union-attr]
...
async def test_lifespan_starts_reconciler_after_bootstrap_and_stops_it(
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(main, "_bootstrap_admin", lambda: events.append("admin"))
    ...
    async with main.lifespan(app):
        ...
        assert events == ["adapter", "database", "admin", "worker-started"]
        assert app.state.payment_reconciler_task is not None

    assert events[-1] == "worker-stopped"


async def test_lifespan_fails_closed_when_admin_wallet_validation_fails(monkeypatch) -> None:
    from blindport import main
    from blindport.adapters.base import NwcAdapterError

    events: list[str] = []

    class InvalidAdminWallet:
        def validate_connection(self, nwc_uri: str) -> None:
            raise NwcAdapterError("unauthorized", "wallet is unauthorized", retryable=False)
    ...
    monkeypatch.setattr(main, "get_nwc_adapter", lambda: InvalidAdminWallet())
    monkeypatch.setattr(main, "get_lnemail_adapter", lambda: events.append("lnemail"))
    monkeypatch.setattr(main, "prepare_database", lambda: events.append("database"))

    with pytest.raises(NwcAdapterError, match="unauthorized"):
        async with main.lifespan(FastAPI()):
            pass

    assert events == ["lightning", "lnemail"]
