...
def test_payment_reminders_default_off() -> None:
    settings = Settings(_env_file=None)

    assert settings.REMINDER_EMAIL_ENABLED is False
    assert settings.LNEMAIL_MAX_SEND_PRICE_SATS == 100
...
@pytest.mark.parametrize(
    ...
    [
        "http://lnemail.net",
        "https://lnemail.net:8443",
        "https://user@lnemail.net",
        "https://lnemail.net/api",
        "https://lnemail.net?target=internal",
    ],
)
def test_lnemail_url_must_be_an_exact_https_origin(value: str) -> None:
    with pytest.raises(ValidationError, match="LNEMAIL_BASE_URL"):
        Settings(_env_file=None, LNEMAIL_BASE_URL=value)


def test_enabled_payment_reminders_require_operator_credentials() -> None:
    with pytest.raises(ValidationError, match="LNEMAIL_ACCESS_TOKEN"):
        Settings(
            _env_file=None,
...
def test_production_payment_reminders_require_real_nwc_adapter() -> None:
    with pytest.raises(ValidationError, match="PAYMENT_NWC_ADAPTER"):
        _production_settings(
            REMINDER_EMAIL_ENABLED=True,
            CREDENTIAL_ENCRYPTION_KEY="cd" * 32,
            LNEMAIL_ACCESS_TOKEN="lnemail-token",
            LNEMAIL_ADMIN_NWC_URI="nostr+walletconnect://admin",
            PAYMENT_NWC_ADAPTER="mock",
        )


def test_payment_reminders_require_reconciliation_worker() -> None:
...
def test_reminder_lease_covers_provider_timeouts() -> None:
    with pytest.raises(ValidationError, match="REMINDER_DELIVERY_LEASE_SECONDS"):
        Settings(
            _env_file=None,
            LNEMAIL_REQUEST_TIMEOUT_SECONDS=20,
