...
def test_existing_pending_txt_claim_retains_legacy_verification(app_client) -> None:
    ...
    with Session(engine) as session:
        stored = session.get(Subscription, created["id"])
...
def test_expired_claim_is_released_and_reclaimable(app_client) -> None:
    ...
    with Session(engine) as session:
        stored = session.get(Subscription, first["id"])
        assert stored is not None
    ...
    with Session(engine) as session:
        expired = session.get(Subscription, first["id"])
...
def test_expiry_releases_unpaid_managed_verified_and_unverified_claims(app_client) -> None:
    ...
    with Session(engine) as session:
        verified_row = session.get(Subscription, verified["id"])
        ...
        for sub_id in (managed["id"], verified["id"], unverified["id"]):
            row = session.get(Subscription, sub_id, populate_existing=True)
            assert row is not None
...
def test_provider_settlement_wins_before_initial_claim_release(app_client, method: str) -> None:
    ...
    with Session(engine) as session:
        stored_sub = session.get(Subscription, sub["id"])
...
def test_processing_payment_and_failed_reconciliation_retain_elapsed_claim(
    ...
    with Session(engine) as session:
        for sub_id, payment_id in (
            ...
            stored_sub = session.get(Subscription, sub_id)
        ...
        for sub_id in (processing_sub["id"], uncertain_sub["id"]):
            retained = session.get(Subscription, sub_id, populate_existing=True)
...
def test_reaper_never_releases_live_domain_claims(app_client) -> None:
    ...
    with Session(engine) as session:
        stored_initial = session.get(Subscription, initial["id"])
        stored_verified = session.get(Subscription, verified["id"])
        stored_active = session.get(Subscription, active["id"])
        stored_grace = session.get(Subscription, grace["id"])
...
def test_expiry_immediately_deauthorizes_but_retains_domain_during_grace(app_client) -> None:
    ...
    with Session(engine) as session:
        stored = session.get(Subscription, active["id"])
        assert stored is not None
...
def test_provider_settlement_wins_before_expired_domain_release(app_client) -> None:
    ...
    elapsed = datetime.now(UTC) - timedelta(seconds=1)
    with Session(engine) as session:
        stored_sub = session.get(Subscription, active["id"])
