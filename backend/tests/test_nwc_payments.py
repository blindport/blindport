...
def _create_nwc(client, token: str, subscription_id: str):
    return client.post(
        "/api/v1/payments",
...
def test_auto_renew_skips_suspended_and_legacy_admin_accounts(app_client) -> None:
    client, factory = app_client
    token, subscription = _wallet_subscription(client)
    initial = client.post(
        "/api/v1/payments",
    ...
    from blindport.core.models import Payment, PaymentMethod, User
    from blindport.db import engine
    from blindport.services.payment_reconciliation import reconcile_pending_payments_once
    ...
    assert reconcile_pending_payments_once().auto_renewed == 0
    ...
    assert reconcile_pending_payments_once().auto_renewed == 0
    with Session(engine) as session:
        assert session.exec(select(Payment).where(Payment.method == PaymentMethod.NWC)).all() == []
