...
def test_deleting_preference_cancels_unfunded_delivery_and_is_idempotent(
    ...
    from blindport.db import engine
    from blindport.services.reminders import queue_payment_reminder
    ...
    with Session(engine) as session:
        ...
        queue_payment_reminder(session, subscription, ReminderKind.SEVEN_DAY)
        session.commit()
...
def test_replacing_preference_cancels_invoice_for_previous_recipient(
    app_client,
    monkeypatch,
) -> None:
    from datetime import UTC, datetime, timedelta
    ...
    with Session(engine) as session:
        ...
        delivery.invoice_ciphertext = "v1.previous-recipient-invoice"
        delivery.invoice_key_version = "a" * 32
        delivery.payment_hash = "ab" * 32
    ...
    with Session(engine) as session:
        delivery = session.exec(select(ReminderDelivery)).one()
        assert delivery.state == ReminderDeliveryState.CANCELLED
        assert delivery.invoice_ciphertext is None
        assert delivery.payment_hash is None
