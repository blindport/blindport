...
def test_order_assets_use_anonymous_order_only_without_a_browser_token() -> None:
    ...
    assert "accounts.setActive(account.token, account.accountId)" in login
    assert "accounts.forget(account.token)" in login
    assert "if (!accounts.forget(account.token))" in login
    ...
    assert 'name="paymentTerm-{{ s.id }}"' in _asset("templates/dashboard.html")
...
def test_templates_have_accessible_external_only_structure() -> None:
    ...
    assert '<a href="/terms">Service terms</a>' in base
