...
document.getElementById("newSubForm").addEventListener("submit", async (event) => {
  ...
  const referralAddress = window.BlindportReferrals?.address();
  if (referralAddress) body.referral_address = referralAddress;
...
function preparePaymentPanel(method) {
  ...
  document.getElementById("paymentDeadlineWarning").hidden = true;
  document.getElementById("paymentDeadlineWarning").textContent = "";
  document.getElementById("paymentReviewNotice").hidden = true;
  document.getElementById("paymentReviewNotice").textContent = "";
...
function showPaymentNotices(payment) {
  const review = document.getElementById("paymentReviewNotice");
  if (payment.settlement_review_required) {
    review.textContent = "Payment verified after checkout or after eligibility changed. Contact support for settlement review.";
    review.hidden = false;
  }
  if (!["lightning", "stablecoin_swap"].includes(payment.method) || !payment.expires_at) return;
  const checkoutDeadline = Date.parse(payment.expires_at);
  if (Number.isNaN(checkoutDeadline)) return;
  const providerDeadline = Date.parse(payment.invoice_expires_at);
  const providerMayOutlastCheckout =
    !Number.isNaN(providerDeadline) && providerDeadline > checkoutDeadline;
  const warning = document.getElementById("paymentDeadlineWarning");
  warning.textContent =
    `Checkout deadline: ${new Date(checkoutDeadline).toLocaleString()}. ` +
    "Do not pay this invoice after the checkout deadline" +
    (providerMayOutlastCheckout ? ", even if the provider invoice remains payable longer." : ".");
...
function renderManualPayment(payment, status) {
  ...
  showPaymentNotices(payment);
  if (payment.settlement_review_required) {
    hidePayableInvoice();
    status.textContent = "Payment received and awaiting settlement review.";
    return;
  }
  if (["expired", "failed"].includes(payment.status)) {
    hidePayableInvoice();
    status.textContent = "Checkout expired. Do not pay the old invoice.";
    return;
  }
  if (!payment.invoice) {
    hidePayableInvoice();
    status.textContent = "Waiting for invoice creation. If it cannot complete, wait for this checkout to expire before trying again.";
    return;
  }
  const payUri = document.getElementById("payUri");
  if (stablecoin) {
  ...
  status.textContent = `Waiting for Lightning payment (${payment.billing_term}, ${payment.period_days} days).`;
}
...
async function pollPayment(payment, status) {
  ...
  while (Date.now() < deadline) {
    ...
    if (current.settlement_review_required) {
      hidePayableInvoice();
      showPaymentNotices(current);
      status.textContent = "Payment received and awaiting settlement review. Contact support.";
      return false;
    }
    ...
    if (!payment.invoice && current.invoice && ["lightning", "stablecoin_swap"].includes(current.method)) {
      renderManualPayment(current, status);
      payment = current;
    }
  }
  hidePayableInvoice();
  status.textContent = "Checkout expired. Do not pay the old invoice. Create a new payment to try again.";
  return false;
}

function hidePayableInvoice() {
  document.getElementById("payBolt11").textContent = "";
