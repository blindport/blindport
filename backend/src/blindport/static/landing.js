...
const referralLinkForm = document.getElementById("referralLinkForm");
if (referralLinkForm) {
  referralLinkForm.addEventListener("submit", (event) => {
    event.preventDefault();
    const input = document.getElementById("referralLinkAddress");
    const output = document.getElementById("referralLinkOutput");
    const status = document.getElementById("referralLinkStatus");
    const address = window.BlindportReferrals?.validateAddress(input.value);
    if (!address) {
      output.value = "";
      output.hidden = true;
      status.textContent = "Enter a valid Lightning Address.";
      return;
    }
    output.value = `${referralLinkForm.dataset.publicOrigin}/#ref=${encodeURIComponent(address)}`;
    output.hidden = false;
    status.textContent = "";
  });
}
