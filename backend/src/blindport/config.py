...
class Settings(BaseSettings):
    ...
    # LNURL receives merchant payments; LND remains available for draining legacy invoices.
    # mocks remain the default for local development and tests.
    PAYMENT_LIGHTNING_ADAPTER: str = "mock"
    LNURL_MERCHANT_ADDRESS: str = "blindport@coinos.io"
    LNURL_REQUEST_TIMEOUT_SECONDS: float = Field(default=10.0, ge=1, le=30)
    LNURL_CHECKOUT_SECONDS: int = Field(default=600, ge=60, le=3600)
    REFERRAL_COMMISSION_BPS: int = Field(default=1000, ge=0, le=10000)
    ...
    @field_validator("LNURL_MERCHANT_ADDRESS")
    @classmethod
    def validate_lnurl_merchant_address(cls, value: str) -> str:
        from .core.lightning_address import validate_lightning_address

        value = validate_lightning_address(value)
        if value.split("@")[1] != "coinos.io":
            raise ValueError("LNURL_MERCHANT_ADDRESS must use coinos.io")
        return value
    ...
    def _validate_production(self) -> None:
        ...
        if self.PAYMENT_LIGHTNING_ADAPTER.lower() not in {"lnd", "lnurl"}:
            failures.append("PAYMENT_LIGHTNING_ADAPTER must use the lnurl or lnd adapter")
        ...
        if self.PAYMENT_LIGHTNING_ADAPTER.lower() == "lnd" and not self.LND_INVOICE_HMAC_KEY:
