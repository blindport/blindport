...
def _compatible(order: AgentOrder, subscription: Subscription, spec: AgentOrderSpec) -> bool:
    return (
        ...
        and subscription.referral_address == spec.referral_address
    )
...
def _get_or_create_order(
    ...
    if existing is not None:
        ...
        if not _compatible(existing, subscription, spec):
            session.rollback()
    ...
    try:
        subscription = subs_svc.create_subscription(
            ...
            referral_address=spec.referral_address,
            commit=False,
            reap_domains=False,
        )
