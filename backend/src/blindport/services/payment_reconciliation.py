...
def reconcile_pending_payments_once(batch_size: int | None = None) -> ReconciliationSummary:
    ...
    if reconcilable_methods or settings.PAYMENT_LIGHTNING_ADAPTER.lower() == "lnurl":
        with Session(engine) as session:
            payment_ids = list(
                session.exec(
                    select(Payment.id)
                    .where(
                        or_(
                            ...
                            and_(
                                ...
                                Payment.status.in_((PaymentStatus.EXPIRED, PaymentStatus.FAILED)),  # type: ignore[union-attr]
                                Payment.settlement_verified_at.is_(None),  # type: ignore[union-attr]
                                Payment.invoice_expires_at >= now - timedelta(days=1),
                            ),
                        ),
                        (Payment.invoice_provider == "lnurl")
                    ...
                    .order_by(
                        func.coalesce(Payment.lnurl_last_checked_at, Payment.created_at), Payment.id
                    )
    ...
    for payment_id in payment_ids:
        try:
            with Session(engine) as session:
                ...
                if (
                    payment.invoice_provider != "lnurl"
                    and payment.method != PaymentMethod.STABLECOIN_SWAP
