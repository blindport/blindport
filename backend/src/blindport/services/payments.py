...
from .referrals import credit_settled_payment
...
def _stage_lightning_invoice(
    ...
    if settings.PAYMENT_LIGHTNING_ADAPTER.lower() == "lnurl":
        ...
        configured = (
            ...
            else settings.LNURL_CHECKOUT_SECONDS
        )
        payment.expires_at = _local_expiry(
            eligibility_deadline, min(configured, invoice_expiry or configured)
        )
        session.add(payment)
        session.commit()
        return
...
def ensure_lightning_invoice(session: Session, payment: Payment) -> Payment:
    ...
    if payment.status != PaymentStatus.PENDING or payment.invoice:
        return payment
    if payment.invoice_provider == "lnurl":
        return _ensure_lnurl_invoice(session, payment)
    if settings.PAYMENT_LIGHTNING_ADAPTER.lower() == "lnurl":
        raise PaymentProviderError(
            "legacy invoices must be reconciled with their original LND provider"
        )
...
def _ensure_lnurl_invoice(session: Session, payment: Payment) -> Payment:
    """Commit the single creation attempt before crossing the non-idempotent boundary."""
    deadline = _aware(payment.expires_at)
    if deadline is None:
        raise PaymentProviderError("LNURL payment has no checkout deadline")
    if deadline <= _utcnow():
        return _expire_payment(session, payment)
    result = session.execute(
        update(Payment)
        .where(
            ...
            Payment.status == PaymentStatus.PENDING,
            Payment.invoice.is_(None),  # type: ignore[union-attr]
            Payment.lnurl_attempted_at.is_(None),  # type: ignore[union-attr]
        )
        .values(lnurl_attempted_at=_utcnow())
        .execution_options(synchronize_session=False)
    )
    claimed = result.rowcount == 1
    session.commit()
    if not claimed:
        # A crash or timeout proves nothing about creation. Never issue again.
        session.execute(
            update(Payment)
            .where(Payment.id == payment.id)
            .values(lnurl_last_checked_at=_utcnow())
            .execution_options(synchronize_session=False)
        )
        session.commit()
        return _reload_payment(session, payment.id)
    try:
        invoice = get_lnurl_adapter().create_invoice(payment.amount_sats)
    except Exception as error:
        raise PaymentProviderError(
            "LNURL invoice creation failed; this attempt will not be retried"
        ) from error
    # A slow response is never published beyond the local checkout window.
    # Persist it anyway so reconciliation can observe any provider-side payment.
    result = session.execute(
        update(Payment)
        .where(
            Payment.id == payment.id,
        ...
        .values(
            ...
            lnurl_metadata=invoice.metadata,
            invoice_expires_at=invoice.expires_at,
            expires_at=min(deadline, invoice.expires_at),
        )
        .execution_options(synchronize_session=False)
    )
    session.commit()
    payment = _reload_payment(session, payment.id)
    if _aware(payment.expires_at) <= _utcnow() and payment.status == PaymentStatus.PENDING:
        return _expire_payment(session, payment)
    return payment


def _check_lnurl_payment(session: Session, payment: Payment) -> Payment:
    """Require LUD-21 proof; late receipts are retained for operator review."""
    payment = _reload_payment(session, payment.id)
    if payment.status == PaymentStatus.PAID or payment.settlement_review_required:
        return payment
    if not payment.invoice:
        payment = ensure_lightning_invoice(session, payment)
        if not payment.invoice:
            return payment
    if not payment.payment_hash or not payment.lnurl_verify_url:
        raise PaymentProviderError("LNURL invoice verification data is incomplete")
    now = _utcnow()
    # Shared durable throttle bounds customer polling and replica concurrency.
    result = session.execute(
        update(Payment)
        .where(
            Payment.id == payment.id,
            Payment.settlement_verified_at.is_(None),  # type: ignore[union-attr]
            (
                Payment.lnurl_last_checked_at.is_(None)  # type: ignore[union-attr]
                | (Payment.lnurl_last_checked_at <= now - timedelta(seconds=10))
            ),
        )
        .values(lnurl_last_checked_at=now)
        .execution_options(synchronize_session=False)
    )
    claimed = result.rowcount == 1
    session.commit()
    if not claimed:
        return _reload_payment(session, payment.id)
    try:
        state = get_lnurl_adapter().invoice_state(
            payment_request=payment.invoice,
            payment_hash=payment.payment_hash,
            verify_url=payment.lnurl_verify_url,
        )
    except Exception as error:
        raise PaymentProviderError("LNURL settlement verification is unavailable") from error
    payment = _lock_payment(session, payment.id)
    if payment.status == PaymentStatus.PAID or payment.settlement_review_required:
        return payment
    deadline = _aware(payment.expires_at)
    if state == LightningInvoiceState.SETTLED:
        if (
            payment.status == PaymentStatus.PENDING
            and deadline is not None
            and deadline > _utcnow()
        ):
            try:
                return _finalize_payment(
                    session, payment, PaymentStatus.PENDING, settlement_verified_at=_utcnow()
                )
            except ValueError:
                # A reservation/domain may have ceased to be eligible. Preserve
                # the receipt without granting capacity or commission twice.
                session.rollback()
                payment = _lock_payment(session, payment.id)
        if payment.status == PaymentStatus.PENDING:
            payment = _expire_payment(session, payment)
        session.execute(
            update(Payment)
            .where(Payment.id == payment.id, Payment.status != PaymentStatus.PAID)
            .values(settlement_verified_at=_utcnow(), settlement_review_required=True)
            .execution_options(synchronize_session=False)
        )
        session.commit()
        return _reload_payment(session, payment.id)
    if payment.status == PaymentStatus.PENDING and deadline is not None and deadline <= _utcnow():
        return _expire_payment(session, payment)
    session.rollback()
    payment = _reload_payment(session, payment.id)
    if payment.status == PaymentStatus.PENDING:
        if payment.method == PaymentMethod.NWC and settings.is_payment_method_enabled(
            payment.method
        ):
            return _reconcile_nwc_payment(session, payment)
        if payment.method == PaymentMethod.CLINK and settings.is_payment_method_enabled(
            payment.method
        ):
...
def _lnd_invoice_state(payment: Payment) -> LightningInvoiceState:
    ...
    try:
        if payment.invoice_provider == "lnurl":
            if not payment.invoice or not payment.lnurl_verify_url:
                raise ValueError("incomplete LNURL invoice")
            return get_lnurl_adapter().invoice_state(
                payment_request=payment.invoice,
                payment_hash=payment.payment_hash,
                verify_url=payment.lnurl_verify_url,
            )
        if settings.PAYMENT_LIGHTNING_ADAPTER.lower() == "lnurl":
            raise ValueError("legacy invoice requires its original provider")
...
def create_payment(
    ...
    payment = Payment(
        ...
        referral_address=subscription.referral_address,
        referral_commission_bps=(
            settings.REFERRAL_COMMISSION_BPS if subscription.referral_address else 0
        ),
    )
    session.add(payment)
    try:
...
def _finalize_payment(
    ...
    **payment_values: object,
    ...
    if payment.invoice_provider == "lnurl" and "settlement_verified_at" not in payment_values:
        # Customer wallet responses never substitute for merchant LUD-21 proof.
        return _check_lnurl_payment(session, payment)
    payment_id = payment.id
    if payment_id is None:
        raise ValueError("payment has no id")
    paid_at = _utcnow()
    ...
    credit_settled_payment(session, payment)
    session.commit()
    return _reload_payment(session, payment_id)
