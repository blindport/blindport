...
from ..core.auth import current_admin, current_user
...
def put_client_order(
    ...
    return AgentOrderResponse(
        ...
        payment=_payment_to_response(result.payment) if result.payment is not None else None,
        state=result.state,
    )
...
@router.post(
    "/subscriptions/{sub_id}/verify-domain",
...
@router.post("/subscriptions/{sub_id}/auto-renew")
...
def create_payment(
    ...
    sub = session.get(Subscription, body.subscription_id)
    if not sub or sub.user_id != user.id:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "subscription not found")
...
def _set_account_suspension(
    ...
    if suspended and target.id == admin.id:
        raise HTTPException(status.HTTP_409_CONFLICT, "administrators cannot suspend themselves")
...
def suspend_account(
    ...
    return _set_account_suspension(user_id, True, admin, session)
...
def unsuspend_account(
    ...
    return _set_account_suspension(user_id, False, admin, session)
