...
def _ctx(request: Request, **extra) -> dict:
    ...
    referral_commission_bps = settings.REFERRAL_COMMISSION_BPS
    request_origin = (
        public_origin
    ...
    base = {
        ...
        "referral_commission_percent": f"{referral_commission_bps / 100:g}",
    }
    base.update(extra)
    return base
...
def dashboard(request: Request, session: Session = Depends(get_session)) -> HTMLResponse:
    ...
    response = templates.TemplateResponse(
        ...
        _ctx(
            ...
            settlement_reviews=session.exec(
                select(Payment)
                .join(Subscription, Subscription.id == Payment.subscription_id)
                .where(Subscription.user_id == user.id, Payment.settlement_review_required)
