...
def _set_account_suspension(
    ...
    if suspended and target.id == admin.id:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "cannot suspend own admin account")
...
def suspend_account(
    ...
    return _set_account_suspension(account_id, True, admin, session)
...
def unsuspend_account(
    ...
    return _set_account_suspension(account_id, False, admin, session)
