...
class Payment(SQLModel, table=True):
    ...
    # NULL identifies legacy LND/mock rows, never an uninitialized LNURL request.
    invoice_provider: str | None = Field(default=None, max_length=16)
    lnurl_attempted_at: datetime | None = Field(default=None, sa_type=DateTime(timezone=True))
    lnurl_verify_url: str | None = Field(default=None, max_length=2048)
    lnurl_metadata: str | None = None
    invoice_expires_at: datetime | None = Field(default=None, sa_type=DateTime(timezone=True))
    settlement_verified_at: datetime | None = Field(default=None, sa_type=DateTime(timezone=True))
    settlement_review_required: bool = Field(
        default=False, sa_column_kwargs={"server_default": text("false")}
    )
    lnurl_last_checked_at: datetime | None = Field(default=None, sa_type=DateTime(timezone=True))
    referral_address: str | None = Field(default=None, max_length=254)
    referral_commission_bps: int = Field(default=0, sa_column_kwargs={"server_default": text("0")})
...
class ReferralPayout(SQLModel, table=True):
    __table_args__ = (
        CheckConstraint("amount_sats >= 10000", name="ck_referralpayout_minimum"),
        CheckConstraint("status IN ('reserved', 'paid')", name="ck_referralpayout_status"),
    )

    id: UUID = Field(default_factory=uuid4, primary_key=True)
    address: str = Field(max_length=254, index=True)
    amount_sats: int
    status: str = Field(default="reserved", max_length=16)
    external_reference: str | None = Field(default=None, max_length=128, unique=True)
    created_at: datetime = Field(default_factory=_utcnow, sa_type=DateTime(timezone=True))
    paid_at: datetime | None = Field(default=None, sa_type=DateTime(timezone=True))


class ReferralCredit(SQLModel, table=True):
    __table_args__ = (CheckConstraint("amount_sats > 0", name="ck_referralcredit_positive"),)

    payment_id: int = Field(primary_key=True, foreign_key="payment.id")
    address: str = Field(max_length=254, index=True)
    amount_sats: int
    payout_id: UUID | None = Field(default=None, foreign_key="referralpayout.id", index=True)
    created_at: datetime = Field(default_factory=_utcnow, sa_type=DateTime(timezone=True))
