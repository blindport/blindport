...
never returned after submission. LNemail BOLT11 invoices are encrypted too because a
provider invoice description can contain the recipient. LNemail access and the operator NWC URI must be
provided through owner-only secret files, and `LNEMAIL_MAX_SEND_PRICE_SATS` bounds
every outgoing message payment before the NWC helper is called. Blindport sends the
recipient and generated reminder text to LNemail, so LNemail's storage and retention
policy remains part of the feature's privacy boundary.
