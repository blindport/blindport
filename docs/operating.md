...
durable LNemail outbox. Keep `REMINDER_EMAIL_ENABLED=false` while migrating. To
enable it, mount `LNEMAIL_ACCESS_TOKEN` and a separate budget-limited
`LNEMAIL_ADMIN_NWC_URI` through secret files, use the production `nwc` adapter and
an exact relay-host allowlist, and retain `LNEMAIL_MAX_SEND_PRICE_SATS=100` unless
the provider's documented send price changes. Recipient addresses use the same
rotatable credential keyring as account NWC credentials but a distinct AES-GCM AAD
purpose. The database stores no recipient, subject, or body plaintext. Lost send
creation responses are not retried; ambiguous wallet payments are looked up by
payment hash before retry. The BOLT11 is also encrypted at rest because LNemail may
include the recipient in its invoice description. LNemail necessarily receives the
recipient and generated message in plaintext and may retain outgoing-send records
under its own policy. Disabling reminders cancels queued and created-but-unpaid work;
it cannot retract a request already being created or a payment already submitted.
...
2. **Customer-owned CNAME verification:** a non-apex customer subdomain receives
   ...
   `POST /api/v1/subscriptions/{id}/verify-domain` remains available for immediate
