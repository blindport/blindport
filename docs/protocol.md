...
Relays fall back through v2 to v1 during rolling deployment. Missing SMTP exceptions
remain fail-closed, and missing prefix bindings disable accounting. The backend revision
digest covers peers, managed inventory, and the SMTP exception set. Firewall policy is
applied before newly authorized routes are activated.
