...
def _create_payment(token: str, sub_id: str, method: str = "lightning") -> dict:
    r = httpx.post(
...
def _wait_for_background_activation(token: str, subscription_id: str) -> None:
    ...
    try:
        ...
        origin = subprocess.Popen(
            [
                ...
                "python",
                "/repo/tests/e2e/routed_origin.py",
            ],
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT,
        )
...
def test_wireguard_ip_end_to_end():
    ...
    with _spawn_wireguard_client(token) as namespace:
        ...
        udp = httpx.get(f"{ROUTED_REQUESTER}/udp", timeout=5)
        assert udp.status_code == 200
        assert udp.content == b"blindport-routed-udp"
        icmp = httpx.get(f"{ROUTED_REQUESTER}/icmp", timeout=5)
