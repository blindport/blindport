...
Exercises the production-ish path:
  ...
  6. Confirms the subscription is active and a public socket got allocated.
...
def _create_cashu_payment(token: str, sub_id: str) -> dict:
    r = httpx.post(
...
def test_cashu_payment_settles_subscription():
    ...
    # Subscription should now be active and have a shared socket assigned.
    cfg = httpx.get(
        f"{BACKEND}/api/v1/client/config", headers=_auth(token), timeout=5
    ).json()
    assert any(
        row["product"] == "port" and row["assigned_ip"] and row["assigned_port"]
        for row in cfg
    ), cfg
