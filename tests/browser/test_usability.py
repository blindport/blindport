...
def test_referral_generator_and_first_touch_session_attribution(
    browser: Browser,
    ...
    try:
        page.goto(
            f"{browser_server.base_url}/#ref=Alice%40Wallet.Co",
            wait_until="networkidle",
        )
        assert (
            page.evaluate("sessionStorage.getItem('blindport_referral_address')")
            == "alice@wallet.co"
        )
        assert "#ref=" not in page.url
        origin = page.locator("#referralLinkForm").get_attribute("data-public-origin")
        page.locator("#referralLinkAddress").fill("Alice@Wallet.Co")
        page.locator("#referralLinkForm").get_by_role(
            "button", name="Create referral link"
        ).click()
        assert (
            page.locator("#referralLinkOutput").input_value()
            == f"{origin}/#ref=alice%40wallet.co"
        )
        for fragment in ("bob%40wallet.co", "not-a-lightning-address"):
            page.goto(
                f"{browser_server.base_url}/#ref={fragment}", wait_until="networkidle"
            )
        assert (
            page.evaluate("sessionStorage.getItem('blindport_referral_address')")
            == "alice@wallet.co"
        ...
        assert (
            order_response.request.post_data_json["referral_address"]
            == "alice@wallet.co"
        )
        page.goto(
            f"{browser_server.base_url}/dashboard#ref=bob%40wallet.co",
            wait_until="networkidle",
        )
        assert (
            page.evaluate("sessionStorage.getItem('blindport_referral_address')")
        ...
        assert "#ref=" not in page.url
        assert all("wallet.co" not in request for request in requests)
        for state in ("pending", "expired", "review"):
            page.evaluate(
                """state => renderManualPayment({
                  method: 'lightning', status: state === 'review' ? 'expired' : state,
                  settlement_review_required: state === 'review',
                  amount_sats: 1000, billing_term: 'monthly', period_days: 30,
                  invoice: state === 'pending' ? null : 'lnbc1oldinvoice',
                  expires_at: new Date(Date.now() + 60000).toISOString()
                }, document.getElementById('payStatus'))""",
                state,
            )
            assert page.locator("#payUri").get_attribute("href") is None
            assert page.locator("#payBolt11").inner_text() == ""
        assert "awaiting settlement review" in page.locator("#payStatus").inner_text()
    finally:
        context.close()
...
def test_dashboard_creates_prorated_wildcard_upgrade(
    ...
    try:
        ...
        with page.expect_response(
            lambda response: (
                response.url.endswith("/wildcard-upgrade")
                and response.request.method == "POST"
            )
